...
const ProjectForm: React.FC<ProjectFormProps> = ({
    ...
    const checkScope = async () => {
        try {
            // We can get scope from a dummy call to /projects or a specific endpoint
            const resp = await jobService.getAllProjectByScope({ pageSize: 1 });
            if (resp?.data?.scope) {
            ...
            console.error('Failed to check scope:', err);
        }
    };
