...
MiniFASNetV1SE (Crop 4.0) + MiniFASNetV2 (Crop 2.7)

Logic tối giản — dùng RAW score + ngưỡng cứng từ rate_config.py:
  v1_raw < LIVENESS_V1_THRESHOLD → FAKE
  v2_raw < LIVENESS_V2_THRESHOLD → FAKE
  Qua cả 2 → REAL
"""
...
class FaceLivenessDetector:
    ...
    def _load_models(self):
        ...
        v1_path = find(paths_v1)
        v2_path = find(paths_v2)
    ...
    # ------------------------------------------------------------------
    # MAIN — Logic tối giản: RAW score + ngưỡng cứng
    # ------------------------------------------------------------------
    def check_liveness(
        self,
        ...
        try:
            ...
            if crop_v1 is None:
                return LivenessResult(False, 0.0, "FAKE", "Crop failed (V1)")
            v1_raw = self._infer(self.v1_session, crop_v1)

            # 3. V2 (Crop 2.7)
            crop_v2 = self._crop_face(face_image, bbox, 2.7)
            if crop_v2 is None:
                return LivenessResult(False, 0.0, "FAKE", "Crop failed (V2)")
            v2_raw = self._infer(self.v2_session, crop_v2)

            # 4. LOG RAW SCORES
            logger.info(
                f"🔍 Liveness RAW: V1={v1_raw:.4%}, V2={v2_raw:.2%} | "
                f"Thresholds: V1>={LIVENESS_V1_THRESHOLD:.2%}, V2>={LIVENESS_V2_THRESHOLD:.2%}"
            )
