...
const MyApplicationListScreen = ({ navigation }) => {
  ...
  const fetchApplications = async (params) => {
    try {
      console.log('📋 [MyApplications] Fetching with params:', params);
  ...
  const handleDelete = (id, type) => {
    Alert.alert(
      '🗑️ Xác nhận xóa',
      `Bạn có chắc chắn muốn xóa đơn "${APPLICATION_TYPE_LABELS[type]}"?`,
      [
      ...
      <Card.Content style={styles.cardContent}>
        <View style={styles.topRow}>
          ...
          <View style={styles.infoSection}>
            ...
            {/* Created Date */}
            <View style={styles.infoRow}>
              <MaterialCommunityIcons name="calendar-plus" size={14} color="#8c8c8c" />
              <Text style={styles.infoText}>
              ...
              {item.status === 0 && (
                <>
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      setVisibleMenuId(null);
                      const screen = getEditScreenForType(item.type);
                      if (screen) {
                        console.log('✏️ [MyApplications] Navigating to EDIT:', screen, 'ID:', item.id);
                        navigation.navigate(screen, { mode: 'edit', applicationId: item.id });
                      } else {
                        Alert.alert('Không hỗ trợ', 'Loại đơn này chưa hỗ trợ chỉnh sửa');
                      }
                    }}
                  >
        ...
        {item.status === 2 && (
          <View style={styles.footerRow}>
            ...
            <Text style={[styles.footerText, { color: '#ff4d4f' }]}>
              {item.data?.rejectReason || 'Đã từ chối'}
            </Text>
          </View>
        )}
      </Card.Content>
